...
var ECSideMessage={
	ENCODING	:	"<comm:message key='js.ec.language'/>",
	WAITTING_MSG	:	"<comm:message key='js.ec.commiting'/>...",
	ERR_PAGENO :	"<comm:message key='js.ec.jump_page'/> #",
	OVER_MAXEXPORT : "<comm:message key='js.ec.total_records_limit'/>( # )"
};
...
var msgTemplate=ECSideMessage[name];
	for (var i=1;i < arguments.length ;i++ ){
		...
		msgTemplate=ECSideUtil.replaceAll(msgTemplate,"<%=symbal%>{"+i+"}",arguments[i]);
	}
	return msgTemplate;
};
...
ECSideUtil.changeRowsDisplayed=function(formid,selectObj){
	...
	try {
        ...
        //ecside改变页面显示页数时，自动刷新，兼容firefox浏览器
		document.all['Report'].style.height=eval('Report').document.body.scrollHeight;
	}catch (e2){
		form.submit();
	}
};
...
ECSideUtil.DragobjBodyOldWidth = ECSideUtil.DragECSideObj.ECListHead.parentNode.clientWidth ;
ECSideUtil.DragobjOldWidth=obj.parentNode.clientWidth;
obj.parentTdW=ECSideUtil.DragobjOldWidth;
	obj.mouseDownX=dx;
	...
	try{
		...
		obj.totalWidth = obj.siblingW + ECSideUtil.DragobjOldWidth;
	...
	if (window.isIE){
		ECSideUtil.Dragobj.parentTdW += ECSideConstants.IE_WIDTH_FIX_A;	
		ECSideUtil.Dragobj.totalWidth += ECSideConstants.IE_WIDTH_FIX_B;
		// ECSideUtil.DragobjOldWidth+=ECSideConstants.IE_WIDTH_FIX_A;		
	}else{
		ECSideUtil.Dragobj.parentTdW += ECSideConstants.FF_WIDTH_FIX_A;		// 4 because paddingLeft + paddingRight=4 ???
		ECSideUtil.Dragobj.totalWidth += ECSideConstants.FF_WIDTH_FIX_B;
		// ECSideUtil.DragobjOldWidth+=ECSideConstants.FF_WIDTH_FIX_A;	
	}
...
ECSideUtil.DoResize=function(event){
	...
	document.body.style.cursor = "col-resize";
	var newWidth=ECSideUtil.Dragobj.parentTdW+ e.screenX ;
	
	var way="resizeTable";
	way=ECSideUtil.DragECSideObj.resizeWay;

	ECSideUtil.ResizeWay[way](newWidth);
...
ECSideUtil.ResizeWay={
	"resizeTable":function(newWidth){
		...
		if(newWidth>ECSideUtil.MinColWidth  ) {
			...
			var dwidth= ECSideUtil.Dragobj.parentNode.clientWidth - ECSideUtil.DragobjOldWidth;

			if (ECSideUtil.DragobjBodyOldWidth+dwidth< ECSideUtil.DragECSideObj.listWidth ){
				ECSideUtil.DragECSideObj.ECListHead.parentNode.style.width=(ECSideUtil.DragobjBodyOldWidth+dwidth)+'px';
			}

			//if (ECSideUtil.DragECSideObj.ECListBody.parentNode!=ECSideUtil.DragECSideObj.ECListHead.parentNode) {
			ECSideUtil.DragECSideObj.ECListBody.parentNode.style.width=ECSideUtil.DragECSideObj.ECListHead.parentNode.clientWidth+"px";
			//}
	...
	"resizeSibling":function(newWidth){
		var newSiblingWidth=ECSideUtil.Dragobj.totalWidth-newWidth; 
