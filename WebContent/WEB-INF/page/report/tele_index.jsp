...
function preview(){
	...
	function download(){
		...
		document.getElementById("dataFrame").src = "${root}/teleStati/teleconsultationStatisticalExcel?startDate=" + start + "&endDate=" + end;
		return true;
	}
</script>
</head>
<body style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');" onload="heigthReset(173);" onresize="heigthReset(173);">
	<div id="center">
		<comm:navigator position="report.manager>>meeitng.report.summary" />
		<form name="MyForm" id="MyForm" method="post" action="${root}/teleStati/teleconsultationStatisticalPreview" onsubmit="return preview();">
		...
		<div id="main">
			<c:if test="${summary != null}">
				<table id="shenqing" class="submit_table" align="center" width="100%"  border="1" >
					<caption><comm:message key="meeting.consultation.summaryReport"/></caption>
					<col width="8%"/>
					<col width="13%"/>
					<col width="17%"/>
					<col width="20%"/>
			...
			<c:if test="${detail != null}">
				<table id="shenqing" class="submit_table" align="center" width="100%" border="1">
				<caption><comm:message key="meeting.consultation.detailReport"/></caption>
				<col width="5%"/>
				<col width="13%"/>
				<col width="12%"/>
				<col width="15%"/>
		         		...
		         		<c:forEach items="${vDetailVO.comms}" var="comm" varStatus="status">
			         		    ...
			         		    <tr class="even">
				         			<c:if test="${status.index == 0}">
					         			...
					         			<td rowspan="${fn:length(vDetailVO.comms)}" align="left">${vDetailVO.submitCompany.name}</td>
