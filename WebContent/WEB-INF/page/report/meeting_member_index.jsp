...
<script type="text/javascript">
	...
	function download(){
		...
		document.getElementById("dataFrame").src = "${root}/statsMeetingMemberReport/statsMeetingMemberByPeopleCountExcel?startDate=" + start + "&endDate=" + end;
		return true;
	}
</script>
</head>
<body style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');" onload="heigthReset(173);" onresize="heigthReset(173);">
	<div id="center">
		<comm:navigator position="report.manager>>meeting.video.member" />
		<form name="MyForm" id="MyForm" method="post" action="${root}/statsMeetingMemberReport/videoLecturesStatisticalPreview" onsubmit="return preview();">
		...
		<div id="main">
		   ...
		   <table id="shenqing" class="submit_table" align="center" width="100%" border="1">
		        <caption><comm:message key="meeting.report.video.member"/></caption>
		        <col width="20%"/>
		        <col width="50%"/>
		        <col width="30%"/>
		        <thead>
         	           ...
         	           <c:forEach items="${statsMeetingMemberlist}" var="meetMember">
	         	        <c:if test="${meetMember.number%2==0}">
		         	        <tr  style="background-color: #f3f4f3;" onmouseover="this.style.backgroundColor='#ecfbd4'" onmouseout="this.style.backgroundColor='#f3f4f3'">
				         	    ...
				         	    <td align="left">${meetMember.name}</td>
				         	    <td align="center">${meetMember.statsCount}</td>
		         	         </tr>
		         	    </c:if>
		         	    <c:if test="${meetMember.number%2!=0}">
