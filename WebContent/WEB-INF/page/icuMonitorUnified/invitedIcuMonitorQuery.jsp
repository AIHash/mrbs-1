...
<head>
	...
	<title><comm:message key="common.icMonit.invitedIcuMonitor" /></title>
			...
			jConfirm(s, "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm){
			    	$.post('${pageContext.request.contextPath}/unifiedindex/acceptedOrRefused?meetingId=' + meetingId+'&flag='+flag, function(text) {
						if(text=='succ'){
							window.location.href = "<%=request.getContextPath()%>/icumonitor/invitedIcuMonitorQuery";
						} else if(text=='error'){
...
<body onload="heigthReset(64);" onresize="heigthReset(64);" style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');">
	<div id="center">
		<comm:navigator position="manager.icuMonit>>common.icMonit.invitedIcuMonitor" />
		<div id="main">
		<form name="MyForm" action="${root}/icumonitor/invitedIcuMonitorList" method="post" target="Report" onsubmit="return check()">
		...
		<table align="center" width="697">
			...
			<tr>
				<td colspan="3" class="table_middle_bg"><iframe id="dataFrame" align="top" frameborder="0" name="Report" width="700" scrolling="no" src="${root}/icumonitor/invitedIcuMonitorList"></iframe></td>
			</tr>
		</table>
	</form>
	</div>
