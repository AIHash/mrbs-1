	...
	<table width="700" border="0" cellspacing="0" cellpadding="0" height="100%">
		...
		<tr>
			...
			<ec:table tableId="ec"
		        var="data"
				items="report_data"
				action="${root}/icumonitor/arrangedIcuMonitorList?${requestParameter}"
		        title=""
		        width="700"
		        ...
		        sortRowsCallback="">
				<ec:row>
					<ec:column property="iCUMonitorId.patientInfo.name" title="meeting.patientsName" width="25%" sortable="true"/>
					<ec:column property="iCUMonitorId.authorInfo.name" title="unified.medicalRecordSubmitPeople" width="15%" sortable="true"/>
					...
					<ec:column property="null" title="unified.operater" sortable="false"  width="35%">
						<a style="color:blue;" href="<%=request.getContextPath()%>/icumonitor/arrangedmeetView/${data.id}" class='various3'><comm:message key='unified.detail'/></a>
						<c:choose>
...
<%-- 								<c:if test="${fn:length(data.satisfactions) == 0}"> --%>
                                   <c:if test="${data.state == MEETING_STATE_END }">
                                    <a style="color:blue;" href="${root}/icumonitor/meetingOpintionForUnified?opintionmeetingid=${data.id}&${requestParameter}" class='various3'><comm:message key='unified.opintion'/></a>
                                   </c:if>
								   <c:if test="${data.state != MEETING_STATE_END }">
