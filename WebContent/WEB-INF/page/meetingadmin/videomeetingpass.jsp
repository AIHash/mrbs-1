...
<script type="text/javascript"  >
	...
	function passsubmit(){
		if(check()){
			var videoMeetingApplicationId=$('#videoMeetingApplicationId').val();
			...
			$.ajaxSettings.traditional = true;
			$.ajax({type :'POST',  url : '${pageContext.request.contextPath}/meeadmdbd/auditVideoLectures', 
				data :  {videoMeetingApplicationId:videoMeetingApplicationId,startTime:starttime,endTime:endtime,selectedUserIds:selectedUserIds,meetingRoom:meetingRoom},
				dataType : 'json',
				dataFilter : function(json, type){
					if(json.indexOf('<html>') != -1){
						window.location.href="<%=request.getContextPath()%>/index.jsp?message=system.session_expire";
				...
				if(json.flag){
					jAlert("<comm:message key='meeting.m.submitsucc'/>", "<comm:message key='meeting.m.infotishi'/>", function(r) {
						url="<%=request.getContextPath()%>/meeadIndex/videoLecturesList";
...
<body id="windows" style="overflow-y:auto;">
  <div id="usual1" class="usual">   
   ...
   <div id="tab1">
    <form id="formtongguo" name="formtongguo" method="post" action="<%=request.getContextPath()%>/meeadmdbd/auditVideoLectures" onsubmit="">
    <input type="hidden" name="videoMeetingApplicationId" id="videoMeetingApplicationId" value="${requestmeetid_add}"/>
	     ...
	     <div id="viewappinfor">
            ...
            <div class="subinfor" id="patientinfor1">
				  <table class="submit_table" align="center" width="100%" border="1" >
					...
					<tr >
			            <td  align="right" style="vertical-align:middle;"><strong><comm:message key='unified.and.expert_metting.speakerName'/></strong>:&nbsp;<font color="red">*</font>&nbsp;</td>		
					    <td colspan="3" style="vertical-align:middle;">
					...
					<tr >
			            <td align="right" style="vertical-align:middle;"><strong><comm:message key='meeing.meeting.m.community'/></strong>:&nbsp;<font color="red">*</font>&nbsp;</td>		
					    <td colspan="3" style="vertical-align:middle;">
					    	<table style="border:1px solid #000;margin-left:20px;margin-right:20px;margin-bottom:10px;margin-top:10px;">
								<tr>
									<td bgcolor="#D0D0D0" style="height:22px;border-right:1px solid #000;border-left:1px solid #000;"" >
     ...
     <div id="viewappinfor">
        ...
        <div class="subinfor" id="patientinfor1">
		   <table id="shenqing" class="submit_table" align="center" width="100%" border="1">
		      ...
		      <c:forEach var="videopass" items="${videoes}" varStatus="status">	      
		        <c:choose>
		             <c:when test="${status.count == 1 && status.count % 2 == 1}">
                        <tr>
                           <td align="center">${fn:substring(videopass.startTime, 0, 16)}</td>
                           <td align="center">${fn:substring(videopass.endTime, 0, 16)}</td>		             
		             </c:when>
