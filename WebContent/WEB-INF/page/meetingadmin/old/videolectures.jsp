...
<div id="center">
	...
	<div id="main" >
			<form name="form1" method="post"
				action="<%=request.getContextPath()%>//meeadIndex/videoApplicationSearch">
				<table width="100%" border="0" cellspacing="5" cellpadding="0">
					<tr>
						<td align="right" class="text_left_red2">
							<comm:message key='unified.expectedTimeStart' />：
						</td>
						<td><input type="text" name="expectedTimeStart" id="expectedTimeStart" value="${query_expectedTimeStart}"
						...
						<td align="right" class="text_left_red2"><comm:message
								key='unified.expectedTimeEnd' />：</td>
						<td><input type="text" name="expectedTimeEnd" value="${query_expectedTimeEnd}"
								id="expectedTimeEnd"
								onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})"
								style="border: 1px solid #7F9DB9;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) no-repeat right;"
								readonly="readonly" size="25" />
					...
					<tr>
						...
						<td>
							<select name="deptName.id" id="deptmentid" style="width:160px;" onchange="getChangeDept();"/>
								<option value=""><comm:message key='unified.status.all'/></option>
								<c:forEach var="deptment" items="${basecode['deptment']}">
									<option value="${deptment.id}" <c:if test="${deptment.id==query_deptment}">selected="selected"</c:if>>${deptment.name}</option>
								</c:forEach>
							</select>
						</td>
						<td align="right" class="text_left_red2"><comm:message key='unified.lecture.people.position' />：</td>
						<td>
							<select name="applicationPosition.id" id="positionld" style="width:130px;">
								<option value="-1" selected="selected">	<comm:message key='unified.status.all' /></option>
								<c:forEach var="applicationPosition" items="${basecode['applicationPosition']}">
									<option value="${applicationPosition.id}" <c:if test="${applicationPosition.id==query_applicationPosition}">selected="selected"</c:if>>${applicationPosition.name}</option>
								</c:forEach>
						     </select>
						</td>
					</tr>
	...
	xlsFileName="Reoprt.xls">
		<ec:row>
			...
			<ec:column property="null" title="unified.operater" sortable="false"  width="14%">
				<a style="COLOR:#8E388E;" href="<%=request.getContextPath()%>/meeadmdbd/viewvedioappmeeting/${data.id}"  ><comm:message key='meeting.m.view'/></a>
		    	<c:if test="${data.state == MEETING_APPLICATION_STATE_PENDING }">
			    	&nbsp;<a style="COLOR:#8E388E;" class="various3"  href="<%=request.getContextPath()%>/meeadmdbd/redirctvideomeetingpass?meetingappserchflag=0&requestmeetid=${data.id}" ><comm:message key='meeting.m.through'/></a>
			    	&nbsp;<a style="COLOR:#8E388E;" class="various3"  href="<%=request.getContextPath()%>/meeadmdbd/redirctvideomeetingrefuse?meetingappserchflag=0&refusemeetingid=${data.id}" ><comm:message key='meeting.m.refuse'/></a>              
				</c:if>
			</ec:column>
		</ec:row>
	</ec:table>
