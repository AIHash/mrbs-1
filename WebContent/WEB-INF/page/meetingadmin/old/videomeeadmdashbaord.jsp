...
<div id="head" class="c">
	...
	<div id="navigation">
		<ul>
        	<li class="active"><a href="<%=request.getContextPath()%>/meeadmdbd/listmeetingaplication"><comm:message key='meeting.m.homepage'/></a></li>
  ...
  <tr>
    ...
    <tr>
        <c:if test="${vm_shenhe.state == 1}">
		    ...
		    <td>
		        <c:if test="${vm_shenhe.meetingType.value != 3 }">
			        <a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/viewappmeeting/${vm_shenhe.id}" class="various3"  ><comm:message key='meeting.m.view'/></a>
			    	&nbsp;<a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/redirctmeetingpass?meetingappserchflag=0&requestmeetid=${vm_shenhe.id}" class="various3"><comm:message key='meeting.m.through'/></a>
			    	&nbsp;<a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/redirctrefuse?meetingappserchflag=0&refusemeetingid=${vm_shenhe.id}" class="various3"><comm:message key='meeting.m.refuse'/></a>        
		        </c:if>
		        <c:if test="${vm_shenhe.meetingType.value == 3 }">&nbsp;</c:if>
		   </td>        
        </c:if>
    </tr>  
...
<table id="index-table-2" class="hd" width="691" border="0" cellspacing="0" cellpadding="0">
  <tr>
   ...
   <c:forEach var="vm_shenhetongguo" items="${vm_shenhetongguo}">
    <tr>
    <c:if test="${vm_shenhetongguo.state == 2}">
	    <td>${vm_shenhetongguo.title}</td>
	    <td>${vm_shenhetongguo.requester.name}</td>
	    <td>${vm_shenhetongguo.meetingType.name}</td>
	    <td><fmt:formatDate value="${vm_shenhetongguo.expectedTime}" pattern="yyyy-MM-dd HH:mm" /></td>
	    <td>
	         <c:if test="${vm_shenhetongguo.meetingType.value != 3}">
	             <a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/viewappmeeting/${vm_shenhetongguo.id}" class="various3"><comm:message key='meeting.m.view'/></a>
	         </c:if>
	         <c:if test="${vm_shenhetongguo.meetingType.value == 3}">&nbsp;</c:if>
	    </td>    
    </c:if>
...
<table id="index-table-3" class="hd" width="691" border="0" cellspacing="0" cellpadding="0">
  <tr>
   ...
   <c:forEach var="vm_shenheweitongguo" items="${vm_shenheweitongguo}">

    <tr>
         <c:if test="${vm_shenheweitongguo.state == 3}">
		    <td>${vm_shenheweitongguo.title}</td>
		    <td>${vm_shenheweitongguo.department.name}</td>
		    <td>${vm_shenheweitongguo.requester.name}</td>
		    <td>${vm_shenheweitongguo.meetingType.name}</td>
		    <td><fmt:formatDate value="${vm_shenheweitongguo.expectedTime}" pattern="yyyy-MM-dd HH:mm" /></td>
		    <td>
		         <c:if test="${vm_shenheweitongguo.meetingType.value != 3}">
			         <a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/viewappmeeting/${vm_shenheweitongguo.id}" class="various3">
			              <comm:message key='meeting.m.view'/>
			         </a>         
		         </c:if>
		         <c:if test="${vm_shenheweitongguo.meetingType.value == 3}">&nbsp;</c:if>
		    </td>         
         </c:if> 
  ...
  <tr>
    ...
    <td> 
        <c:if test="${meeting_state_pending.meetingType.value != 3}">
			...
			&nbsp;<c:if test="${meeting_state_pending.state==3}"><a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/redirctsummary?requestsummaryid=${meeting_state_pending.id}" class="various3" ><comm:message key='meeting.m.summary'/></a></c:if>        
        </c:if>
					...
					<table>
						<tr  align="left">
							<td width="65%"><strong><img src="<%=request.getContextPath()%>/resources/images/unified/ico1.gif" width="16" height="16" align="top" />&nbsp;
            					<a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/meetingappsearchlist"><comm:message key='meeting.m.myaccraditation'/></a><span>(${vm_noaccraditationcount_all})</span> </strong>
            				</td>
            				<td width="35%"><comm:message key='meeting.m.toapproval'/><span>(${vm_noaccraditationcount})</span></td>
            			</tr>
            			<tr  align="left">
            				<td width="65%">&nbsp;</td>
            				<td width="35%"><comm:message key='meeting.m.hadapproval'/><span>(${vm_accraditationcount_ytg})</span></td>
						</tr>
					</table>
            	</li>
