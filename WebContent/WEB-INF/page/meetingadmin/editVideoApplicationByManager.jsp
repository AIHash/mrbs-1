...
<script type="text/javascript"> 
	...
	function check(){
		...
		$.ajax({url:'${pageContext.request.contextPath}/meeadmdbd/checkTimeRepeat',
		 	...
		 	dataType : 'text',
		 	async:false,
		 	data: {startTime :startTime, endTime : endTime,meetingRoom : meetingRoom,exceptMeetingId:exceptMeetingId,meetingTypestr:1},
...
<style>
	...
	#rightdown{background-image: url("<%=request.getContextPath()%>/resources/images/table/down.gif");background-repeat: repeat-x;width:19px;height:28px;margin-bottom:2px;margin-left:15px;margin-right:15px;margin-top:8px;}
	#leftup{background-image: url("<%=request.getContextPath()%>/resources/images/table/up.gif");background-repeat: repeat-x;width:19px;height:28px;margin-bottom:2px;margin-left:15px;margin-right:15px;margin-top:20px;}
</style>
</head>
	...
	<div id="center">
		<comm:navigator position="manager.videoLectures.lectureAudit>>unified.video.application.edit" />
		  <div id="main" style=" overflow-y:auto;overflow-x:hidden;width:725px;margin:0px;float:left;">
			<form method="post" action="<%=request.getContextPath()%>/meeadmdbd/editVideoApplicationForPass" onsubmit="return check()">
				...
				<input type="hidden" value="${vmapp.noticeSendTimes}" name="noticeSendTimes"/>
				<input type="hidden" id="selectedstr" name="selectedstr" value="${selectstr }"/>
				
				<c:if test="${vmapp.startTime<=currDate}">
					...
					<table id="shenqing" width="100%" class="shenqing_table" border="0">
							...
							<td width="30%" align="left">
								...
								<c:if test="${vmapp.startTime<=currDate}">
									<input type="text" name="startTime" id="startTime" class="Wdate"  onclick="var endTime=$dp.$('endTime');
										...
										no-repeat right;width:126px;" value="<fmt:formatDate value="${vmapp.startTime}" pattern="yyyy-MM-dd HH:mm" />"readonly="readonly"
									 	disabled="disabled"
									 />
								...
								<c:if test="${vmapp.startTime>currDate && checkUpdateStartTime == false}">
									<input type="text" name="startTime" id="startTime" class="Wdate"  onclick="var endTime=$dp.$('endTime');
										WdatePicker({readOnly:true,onpicked:setEndTime,dateFmt:'yyyy-M-d H:mm',minDate:'%y-%M-%d %H:%m:%s'})" 
										onfocus="this.blur()" style="border:#999 1px solid;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) 
										no-repeat right;width:126px;" value="<fmt:formatDate value="${vmapp.startTime}" pattern="yyyy-MM-dd HH:mm" />"readonly="readonly"
								...
								<c:if test="${vmapp.startTime>currDate && checkUpdateStartTime == true}">
									<input type="text" name="startTime" id="startTime" class="Wdate"  onclick="var endTime=$dp.$('endTime');
										WdatePicker({readOnly:true,onpicked:setEndTime,dateFmt:'yyyy-M-d H:mm',minDate:'%y-%M-%d %H:%m:%s'})" 
										onfocus="this.blur()" style="border:#999 1px solid;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) 
										no-repeat right;width:126px;" value="<fmt:formatDate value="${vmapp.startTime}" pattern="yyyy-MM-dd HH:mm" />"readonly="readonly"
							...
							<td>
								<c:if test="${vmapp.startTime<=currDate}">
									<input type="text" name="endTime" id="endTime" class="Wdate"  onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-M-d H:mm'})" 
										...
										value="<fmt:formatDate value="${vmapp.endTime}" pattern="yyyy-MM-dd HH:mm" />"readonly="readonly" 
									/>		
						...
						<tr>
			                ...
			                <td style="display:none;" width="30%">
									<select name="level.id" id="meetingleveld" style="width:130px;">
						...
						<tr>
						    ...
						    <td colspan="3" style="vertical-align:middle;">
						    	<table style="border:1px solid #000;margin-bottom:10px;margin-top:10px;margin-right:20px;"class="submit_table1" >
									...
									<tr>
										<td style="border-right:1px solid #000;border-bottom:1px solid #000;width:227px;">
											<select multiple="multiple" name="innaiUserSelect" id="innaiUserSelect"  style="align:center;width:227px;height:143px;border:none;">
											</select>	
										</td>
										...
										<td style="border-left:1px solid #000;border-right:1px solid #000;border-bottom:1px solid #000;">
											<select align="center" multiple="multiple" name="innaiUserSelected" id="innaiUserSelected" style="width:220px;height:143px;border:none;">
						...
						<tr >
						    ...
						    <td colspan="3" style="vertical-align:middle;">
						    	<table style="border:1px solid #000;margin-bottom:10px;margin-top:10px;margin-right:20px;"class="submit_table1" >
									...
									<tr>
										<td style="border-right:1px solid #000;border-bottom:1px solid #000;width:227px;">
											<select multiple="multiple" name="commonUserSelect" id="commonUserSelect"  style="align:center;width:227px;height:143px;border:none;border-right:1px">
											</select>	
										</td>
										...
										<td style="border-left:1px solid #000;border-right:1px solid #000;border-bottom:1px solid #000;">
											<select align="center" multiple="multiple" name="commonUserSelected" id="commonUserSelected" style="width:220px;height:143px;border:none;">
						...
						<c:if test="${vmapp.videoRequester.userType.value == USER_TYPE_MEETING_ADMIN}">
							<tr>
						...
						<tr>
							...
							<td colspan="3">
							      <table width="98%" id="videoappinfor" class="_table" cellspacing="0" cellpadding="0" border="1">
							             <tr>
							             ...
							             <tr>
							                ...
							                <c:if test="${fn:length(vmapp.videoRequester.mail) > 30}">
							                  <td title="${vmapp.videoRequester.mail}">${fn:substring(vmapp.videoRequester.mail, 0, 40)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
							                </c:if>							             
							             </tr>
							     </table>		
							</td>
						...
						<tr>
							<td colspan="4" align="center"><input class="button" type="submit" name="button" id="button" value="<comm:message key='comm.save'/>"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
