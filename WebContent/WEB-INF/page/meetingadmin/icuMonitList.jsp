	...
	<table width="700" border="0" cellspacing="0" cellpadding="0" height="100%">
		...
		<tr>
			...
			<ec:table tableId="ec"
		        var="data"
				items="report_data"
				action="${root}/meeadIndex/icuMonitList?${requestParameter}"
		        title=""
		        width="700"
				...
				<ec:row> 
					...
					<ec:column property="null" title="unified.operater" sortable="false"  width="45%">
						<a style="color:blue;" href="<%=request.getContextPath()%>/unified/icuMonitorExamineViewAndInvitedState/${data.id}" class="various3"><comm:message key='meeting.m.view'/></a>
						
						<c:choose>
							<c:when test="${data.state == MEETING_APPLICATION_STATE_PENDING}">
								<a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/icuMonitorEdit/${data.id}" target="WorkBench" ><comm:message key='unified.edit'/> </a>
							</c:when>
							<c:when test="${data.state == MEETING_APPLICATION_STATE_PASS && data.endTime >= currDate}">
								<a style="COLOR:blue;" href="<%=request.getContextPath()%>/meeadmdbd/icuMonitorEditForPass/${data.id}" target="WorkBench"><comm:message key="unified.edit" /></a>
							</c:when>
							<c:otherwise>
								<font color="gray">
						...
						<c:choose>
							<c:when test="${data.state == MEETING_APPLICATION_STATE_PENDING }">
								<a style="color:blue;" class="various3" href="<%=request.getContextPath()%>/icumonitor/redirctIcuMonitorpass?meetingappserchflag=0&requestIcuMonitid=${data.id}"><comm:message key='meeting.m.through'/></a>
								<a style="color:blue;" class="various3" href="<%=request.getContextPath()%>/icumonitor/redirctrefuse?meetingappserchflag=0&refusemeetingid=${data.id}"><comm:message key='meeting.m.refuse'/></a>
								<font color="gray">
							...
							<c:otherwise>
								...
								<c:if test="${data.state == MEETING_APPLICATION_STATE_PASS }">
									<a style="color:blue;" class="various3" href="<%=request.getContextPath()%>/icumonitor/icuMonitorAuditNotice?refusemeetingid=${data.id}"><comm:message key='meeting.noticeName'/>(${data.noticeSendTimes})</a>
									<c:if test="${data.noticeSendTimes>0}">
									    <a style="color:blue;" class="various3" href="<%=request.getContextPath()%>/icumonitor/noticeDetail?refusemeetingid=${data.id}"><comm:message key='meeting.notice.detail'/></a>
									</c:if>
									<c:if test="${data.noticeSendTimes==0}">
									    <font color="gray">
										<comm:message key='meeting.notice.detail'/>
