...
<head>
	...
	<title><comm:message key="manager.icuMonit.consultationAudit" /></title>
	...
	<script type="text/javascript"> 
		function deleteCheck(deptId){
			jConfirm("<comm:message key='js.confirm_delete' args='manager.icuMonit'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm){
			    	$.post('${pageContext.request.contextPath}/meeadmdbd/delIcuMonitor/' + deptId, function(text) {
						if(text == 'succ'){
							window.location.href = "<%=request.getContextPath()%>/meeadIndex/icuMonitQuery";
						} else if(text=='error'){
...
<body onload="heigthReset(64);" onresize="heigthReset(64);" style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');">
	<div id="center">
		<comm:navigator position="manager.icuMonit>>manager.icuMonit.consultationAudit" />
		<div id="main">
		<form name="MyForm" action="${root}/meeadIndex/icuMonitList" method="post" onsubmit="return check()" target="Report">
...
<%-- 						 	<td width="25%"  class="text_left_red2" align="right">
		...
		<table align="center" width="697">
			...
			<tr>
				<td colspan="3" class="table_middle_bg"><iframe id="dataFrame" align="top" frameborder="0" name="Report" width="700" scrolling="no" src="${root}/meeadIndex/icuMonitList"></iframe></td>
			</tr>
		</table>
	</form>
	</div>
