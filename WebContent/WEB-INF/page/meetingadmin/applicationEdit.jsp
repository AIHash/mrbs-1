	  ...
	  function myAlert(){
		...
		function cancelFun(){
			jConfirm("<comm:message key='js.confirm_cancel' args='comm.modify'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm) {
					location='<%=request.getContextPath()%>/meeadIndex/teleconferenceQuery';
				} else {
					return false;
				}
			});
...
<form method="post"   action="<%=request.getContextPath()%>/meeadmdbd/editApplication"  onsubmit="return check()">
