...
<!--
	...
	function setEndTime() {
		...
		if(!strTrim(startTime) == ""){
			 ...
			 var newEndTime=getDate(endDate,"YYYY-MM-DD HH:MM");
			 var cuurDate =document.getElementById("cuurDate").value;
			 if (compareCheckDate(newEndTime,cuurDate) != '-1'){
				 document.getElementById("endTime").value= cuurDate;
			 }else{
	...
	function icdDisplayCheck(){
	  ...
	  function myAlert(){
		...
		function resetValue(){
	        ...
	        var innaiUserSelecteds = $('#innaiUserSelected option').length;
			if(innaiUserSelecteds>0){
	        ...
	        var commonUserSelecteds = $('#commonUserSelected option').length;
	        if(commonUserSelecteds>0){
	        ...
	        var interUserSelecteds = $('#interUserSelected option').length;
	        if(interUserSelecteds>0){
	        	document.getElementById("queryInterUserName").value="";
		        document.getElementById("interUserOldValue").value="";
		        $("#interUserSelected").empty();  
		        changeInterUser();
				document.getElementById("intSelectedInterUser").innerHTML="0";
	        }
...
.titleStyle{color:#3c3645;font-size:18px;font-weight: bold;height:45px;margin-top:15px;margin-bottom:0px;margin-left:40px;border:0px dashed #3c3645;}
...
<body style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');" onload="myAlert();heigthReset(64);" onresize="heigthReset(64);">
	<div id="center">
		<comm:navigator position="consultation.app>>manager.teleconsultation.consultationCollection" />
	<div style="overflow-x:hidden;" id="main" >
<form method="post"   action="<%=request.getContextPath()%>/meeadmdbd/addConsultationCollection" onsubmit="return check();">
	<div id="viewappinfor">
	    <div class="subtitle">
	    ...
	    <div class="subinfor" id="patientinfor1">
	    	...
	    	<input type="hidden" id="cuurDate" name="cuurDate" value="${cuurDate}" />
				...
				<tr>
					...
					<td height="30">
					     <table width="100%" class="meetingApp" border="0">
					             <tr>
					                  <td width="20%"><input type="text" name="patientInfo.name" id="patient_name" size="23" maxlength="20"/> </td>
				...
				<tr>
					...
					<td align="center" width="80%">
					     <table style="width:100%;border:0px solid green;">
					           ...
					           <tr>
					                <td width="32px;">&nbsp;</td>
					           		<td width="200px;" id="hainaAnch"><a href="${HAINA_URL}/HinacomWeb/DisplayImage.aspx?OEM=160&UserID=${USER_LOGIN_SESSION.userId}&UserName=${sessionScope.encodeName}&HospitalID=${USER_LOGIN_SESSION.userId}&UserRetrieve=0" target="_blank" style="color: blue;decoration:underline;">上传患者放射影像</a></td>
					           </tr>
					     </table>
					</td>
				</tr>
						...
						<table style="border:1px solid #000;">
							<tr>
								<td bgcolor="#D0D0D0" style="height:25px;border-right:1px solid #000;">
									 &nbsp;<strong>速查条件</strong><input id="queryDepartmentName" style="align:right;line-height:18px;height:18px;margin-right:2px;margin-left:2px;" value="" size="23" />
								</td>
								<td>
								</td>
				...
				<tr id="icdDefaultDisplay">
					...
					<td>
						<table style="border:1px solid #000;">
							<tr>
								<td bgcolor="#D0D0D0" style="height:25px;border-right:1px solid #000;">
									 &nbsp;<strong>速查条件</strong><input id="queryIcd10Name" style="align:right;line-height:18px;height:18px;margin-right:2px;margin-left:2px;" value="" size="23" />
								</td>
								<td>
								</td>
								<td bgcolor="#D0D0D0"  style="height:25px;border-left:1px solid #000;">
								...
								<td>
									<div class="main1_sub" style="border:0px solid red;">
									        <table width="100%" border="0">
									        	<tr>
									                <td width="55%">
									                	 <input type="text" name="startTime" id="startTime" class="Wdate"  onclick="var endTime=$dp.$('endTime');WdatePicker({readOnly:true,onpicked:setEndTime,dateFmt:'yyyy-M-d H:mm',maxDate:'%y-%M-%d %H:%m:%s'})" onfocus="this.blur()" style="border:#999 1px solid;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) no-repeat right;width:126px;" readonly="readonly" />
									                </td>
									                <td width="55%">
									                     <comm:message key='meeting.m.endtime' />:<span class="required">*</span> 
														 <input type="text" name="endTime" id="endTime" class="Wdate"  onclick="WdatePicker({readOnly:true,dateFmt:'yyyy-M-d H:mm',maxDate:'%y-%M-%d %H:%m:%s'})" onfocus="this.blur()" style="border:#999 1px solid;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) no-repeat right;width:126px;" readonly="readonly" />
									                </td>
									            </tr>
									          </table>
									 </div>
							...
							<tr>
								...
								<td>
									<table style="border:1px solid #000;">
										<tr>
											<td bgcolor="#D0D0D0" style="height:25px;border-right:1px solid #000;">
												 &nbsp;<strong>速查条件</strong><input id="queryInnaiUserName" style="align:right;width:130px;line-height:18px;height:18px;margin-right:2px;margin-left:2px;" value="" size="23" />
											</td>
											<td>
											</td>
											<td bgcolor="#D0D0D0"  style="height:25px;border-left:1px solid #000;">
							...
							<tr>
								...
								<td>
									<table style="border:1px solid #000;">
										<tr>
											<td bgcolor="#D0D0D0" style="height:25px;border-right:1px solid #000;">
												 &nbsp;<strong>速查条件</strong><input id="queryCommonUserName" style="align:right;width:130px;line-height:18px;height:18px;margin-right:2px;margin-left:2px;" value="" size="23" />
											</td>
											<td>
											</td>
											<td bgcolor="#D0D0D0"  style="height:25px;border-left:1px solid #000;">
							...
							<tr>
								...
								<td>
									<table style="border:1px solid #000;">
										<tr>
											<td bgcolor="#D0D0D0" style="height:25px;border-right:1px solid #000;">
												 &nbsp;<strong>速查条件</strong><input id="queryInterUserName" style="align:right;width:130px;line-height:18px;height:18px;margin-right:2px;margin-left:2px;" value="" size="23" />
											</td>
											<td>
											</td>
											<td bgcolor="#D0D0D0"  style="height:25px;border-left:1px solid #000;">
