...
<head>
	...
	<title><comm:message key="manager.videoLectures.lectureEvaluation" /></title>
	...
	<script type="text/javascript"> 
		function check(){
			 ...
			 if (startTime.length> 1&&endTime.length > 1
					&&!compareDateString(startTime, endTime)){
			...
			jConfirm("<comm:message key='js.confirm_delete' args='vedio.app'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm){
			    	$.post('${pageContext.request.contextPath}/meeadmdbd/delMeeting/' + meetingId, function(text) {
						if(text=='succ'){
							window.location.href = "<%=request.getContextPath()%>/meeadIndex/videoLecturesEvaluationQuery";
						} else if(text=='error'){
...
<body onload="heigthReset(64);" onresize="heigthReset(64);" style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');">
	<div id="center">
		<comm:navigator position="manager.videoLectures>>manager.videoLectures.lectureEvaluation" />
		<div id="main">
		<form name="MyForm" action="${root}/meeadIndex/videoLecturesEvaluationList" method="post" onsubmit="return check()" target="Report">
					...
					<table width="695" border="0" align="center" cellspacing="0" cellpadding="0">
						...
						<tr>
						 	...
						 	<td width="30%" align="left">
						 		...
						 		<%-- <select name="departmentName" id="departmentName" style="width:187px;height:20px;">
		...
		<table align="center" width="697">
			...
			<tr>
				<td colspan="3" class="table_middle_bg"><iframe id="dataFrame" align="top" frameborder="0" name="Report" width="700" scrolling="no" src="${root}/meeadIndex/videoLecturesEvaluationList"></iframe></td>
			</tr>
		</table>
	</form>
	</div>
