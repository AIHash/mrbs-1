	  ...
	  function myAlert(){
		...
		function cancelFun(){
			jConfirm("<comm:message key='js.confirm_cancel' args='comm.modify'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm) {
					location='<%=request.getContextPath()%>/unifiedindex/applyForICUMonitQuery';
				} else {
					return false;
				}
			});
...
<form method="post"   action="<%=request.getContextPath()%>/unified/editIcuMontorBegin"  onsubmit="return check()">
					            ...
					            <tr>
					                <td width="30%" style="display: none">
										<select name="meetingType.id" id="meetingtypeid" style="width:180px;">
													...
													<option value="1" <c:if test="${meetingtype.id ==1 }">selected</c:if>  >在线会议</option>
