	...
	<table width="700" border="0" cellspacing="0" cellpadding="0" height="100%">
		...
		<tr>
			...
			<ec:table tableId="ec"
		        var="data"
				items="report_data"
				action="${root}/unifiedindex/applyForICUMonitList?${requestParameter}"
		        title=""
		        width="700"
				...
				<ec:row>
					...
					<ec:column property="null" title="unified.operater" sortable="false"  width="20%">
						<a style="color:blue;" href='${pageContext.request.contextPath}/meeadmdbd/viewappICUMonit/${data.id}' class='various3'><comm:message key="unified.detail"/></a>
						<c:choose>
							<c:when test="${data.state == MEETING_APPLICATION_STATE_PENDING && data.requester.userId == USER_LOGIN_SESSION.userId}">
								<a style="color:blue;" href="${root}/unified/ICUMonitorEdit/${data.id}" target="WorkBench"><comm:message key="unified.edit" /></a>
							</c:when>
							<c:otherwise>
								<font color="gray">
