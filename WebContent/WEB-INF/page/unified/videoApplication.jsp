...
function check(){
	...
	/*  var deptmentid = document.getElementById("deptmentid").value;
	 ...
	 var starttime=document.getElementById("expecttime").value;
	 if(starttime==""){
	...
	<div id="center">
		<comm:navigator position="application.vedio>>application.vedio" />
		  <div id="main" style=" overflow-y:auto;overflow-x:hidden;width:725px;margin:0px;float:left;">
			<form method="post" action="<%=request.getContextPath()%>/unified/addVideoApplicationPurpose" onsubmit="return check()">
					...
					<table id="shenqing" width="100%" class="shenqing_table" border="0">						
						...
						<c:if test="${user.userType.value == 4 || user.userType.value == 5 }">
							<tr>
							   ...
							   <td colspan="3">
							     <input type="text" name="expecttime" id="expecttime" onclick="WdatePicker({dateFmt:'yyyy-M-d H:mm',minDate:'%y-%M-%d %H:%m:%s',maxDate:'{%y+1}-%M-%d 21:00:00'})" style="border: 1px solid #7F9DB9;height:20px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) no-repeat right;" onfocus="this.blur()" size="20" />
							   </td>							                
							</tr>						
						</c:if>
						...
						<tr>
					    	...
					    	<table style="border:1px solid #000;">
								<tr>
									...
									<td bgcolor="#D0D0D0">
										 &nbsp;<strong>速查条件</strong><input id="queryInnaiUserName" style="align:right;line-height:18px;height:18px;width:193px;" value="" size="30"></input>
									</td>
								</tr>
								<tr>
							...
							<td>
							     <table border="0" width="100%">
							           <tr>
						                   <td width="30%">
												<select name="deptName.id" id="deptmentid" style="width:130px;">
							                ...
							                <td>
							                     <select name="userName.userId" id="userName" style="width:130px;"></select>
							                </td>
							           </tr>
							     </table>
							</td> --%>
							<input type="hidden" name="userName.userId" id="userName" value=""/>
						</tr>						
						...
						<tr>
							<td width="15%"  align="right">
								<c:if test="${user.userType.value == 4||user.userType.value == 2}">
							      ...
							      <table width="99%" style="margin-left:0px;" class="_table" cellspacing="0" cellpadding="0" border="0">
							             <tr>
								            ...
								            <c:if test="${fn:substring(user.deptId.parentDeptCode, 0, 6) == '001003'||user.deptId.deptcode == '001003'}">
								                ...
								                <c:if test="${fn:length(user.name) > 10}">
								                <td title="${user.name}">${fn:substring(user.name, 0, 10)}...&nbsp;</td>
								                </c:if>
								                <td align="right" width="15%"><comm:message key='unified.and.expert_username'/>:&nbsp;&nbsp;</td>
								                <td>${user.username}&nbsp;</td>
								            </c:if>
							             ...
							             <tr>
						                    ...
						                    <c:if test="${user.userType.value == 4}">						                    
								                <c:if test="${fn:length(user.mail) <= 30}">
								                  <td>${user.mail}&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
								                <c:if test="${fn:length(user.mail) > 30}">
								                  <td title="${user.mail}">${fn:substring(user.mail, 0, 38)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
						                    </c:if> 
						                    <c:if test="${user.userType.value == 5}">
								                <c:if test="${fn:length(user.mail) <= 26}">
								                  <td>${user.mail}&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
								                <c:if test="${fn:length(user.mail) > 26}">
								                  <td title="${user.mail}">${fn:substring(user.mail, 0, 26)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>						                    
						                    </c:if>
						...
						<tr>
							<td colspan="4" align="center"><input class="button" type="submit" name="button" id="button" value="<comm:message key='comm.save'/>" onclick="beforeSumbit()"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							 <input	class="button" type="reset" name="button2" id="button2"	value="<comm:message key='unified.reset'/>" />
							</td>
						</tr>
					</table>	            
	            </div>
