  ...
  function check(){
	...
	function cancelFun(){
		jConfirm("<comm:message key='js.confirm_cancel' args='comm.modify'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
			if(resultConfirm){
				location='<%=request.getContextPath()%>/unifiedindex/applyForLectureQuery';
			}else {
				return false;
			}	
		});
...
<body style="padding:0; margin:0;background:url('../../resources/images/theme/bg_right-2.gif');" class="bg" onload="heigthReset(64);" onresize="heigthReset(64);">
	<div id="center">
		<comm:navigator position="application.vedio>>unified.video.application.edit" />
		  <div id="main" style=" overflow-y:auto;overflow-x:hidden;width:725px;margin:0px;float:left;">
			<form method="post" action="<%=request.getContextPath()%>/unified/editVideoApplicationPurpose" onsubmit="return check()">
					...
					<table id="shenqing" class="shenqing_table" width="100%" border="0">
						...
						<tr>
		                   ...
		                   <td style="display:none;" width="30%">
								<select name="meetinglevel" id="meetingleveld" style="width:130px;">
							      ...
							      <table width="99%" id="videoappinfor" style="margin-left:0px;" class="_table" cellspacing="0" cellpadding="0" border="1">
							             <tr>
								            ...
								            <c:if test="${fn:substring(user.deptId.parentDeptCode, 0, 6) == '001003'||user.deptId.deptcode == '001003'}">
								            	<td align="right"><comm:message key='group.name'/>:&nbsp;&nbsp;</td>
								                <c:if test="${fn:length(user.name) <= 8}">
								                  <td>${user.name}&nbsp;</td>
								                </c:if>
								                <c:if test="${fn:length(user.name) > 8}">
								                  <td title="${user.name}">${fn:substring(user.name, 0, 8)}...&nbsp;</td>
								                </c:if>						
								                <td align="right"><comm:message key='unified.and.expert_username'/>:&nbsp;&nbsp;</td>
								                <td>${user.username}&nbsp;</td>
								            </c:if>
							             ...
							             <tr>
					                        ...
					                        <c:if test="${user.userType.value == 5}">
								                ...
								                <c:if test="${fn:length(user.mail) > 30}">
								                  <td title="${user.mail}">${fn:substring(user.mail, 0, 30)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
					                        </c:if>
					                        <c:if test="${user.userType.value == 4}">
										        <c:if test="${fn:length(user.mail) <= 28}">
								                  <td>${user.mail}&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
								                <c:if test="${fn:length(user.mail) > 28}">
								                  <td title="${user.mail}">${fn:substring(user.mail, 0, 28)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>
					                        </c:if>					             
							             </tr>
							     </table>		
