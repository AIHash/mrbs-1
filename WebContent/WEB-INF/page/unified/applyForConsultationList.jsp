	...
	<table width="700" border="0" cellspacing="0" cellpadding="0" height="100%">
		...
		<tr>
			...
			<ec:table tableId="ec"
		        var="data"
				items="report_data"
				action="${root}/unifiedindex/applyForConsultationList?${requestParameter}"
		        title=""
		        width="700"
				...
				<ec:row>
					...
					<%-- <ec:column property="applicationTime" title="unified.app.time" cell="date" format="yyyy-MM-dd HH:mm" sortable="true"  width="13%"/> --%>
					<%-- <ec:column property="meetingType.name" title="unified.meetingtype" width="13%" sortable="true"/> --%>
					...
					<%-- <ec:column property="level" title="meeting.m.meetinglevel" sortable="true"  width="12%">
						<c:choose>
					...
					<ec:column property="null" title="unified.operater" sortable="false"  width="20%">
						<a style="color:blue;" href='${pageContext.request.contextPath}/meeadmdbd/viewappmeeting/${data.id}' class='various3'><comm:message key="unified.detail"/></a>
						<c:choose>
							<c:when test="${data.state == MEETING_APPLICATION_STATE_PENDING && data.requester.userId == USER_LOGIN_SESSION.userId}">
								<a style="color:blue;" href="${root}/unified/applictionEdit/${data.id}" target="WorkBench"><comm:message key="unified.edit" /></a>
							</c:when>
							<c:otherwise>
								<font color="gray">
