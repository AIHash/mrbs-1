	  ...
	  function myAlert(){
		...
		function cancelFun(){
			jConfirm("<comm:message key='js.confirm_cancel' args='comm.modify'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm) {
					location='<%=request.getContextPath()%>/unifiedindex/applyForConsultationQuery';
				} else {
					return false;
				}
			});
...
.titleStyle{color:#3c3645;font-size:18px;font-weight: bold;height:28px;line-height:28px;margin-top:15px;margin-bottom:0px;margin-left:40px;}
...
<div id="center">
	<comm:navigator position="application.consultation>>application.consultation.modify" />
	<div style="overflow-x:hidden;" id="main" >
<form method="post"   action="<%=request.getContextPath()%>/unified/edit"  onsubmit="return check()">
					                ...
					                <td width="30%">
										<select name="meetingType.id" id="meetingtypeid" style="width:180px;">
												<c:forEach var="meetingtype" items="${basecode['meetingtype']}">
													<option value="${meetingtype.id}" <c:if test="${meetingtype.id ==app.meetingType.id }">selected</c:if>  >${meetingtype.name}</option>
												</c:forEach>
										</select>					                
					                </td>
								...
								<td align="center" height="80">
							     <table width="470" align="center" style="margin-left:3px;border-bottom:0px;border-right:0px;" class="_table" border="1">
								            ...
								            <tr>
								                ...
								                <c:if test="${fn:length(user.mail) > 21}">
								                  <td align="left" title="${user.mail}">${fn:substring(user.mail, 0, 21)}...&nbsp;<input type="hidden" name="applicantid" id="applicantid"/></td>
								                </c:if>								            
								            </tr>					            
								     </table>
								</td>
