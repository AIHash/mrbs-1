...
<title><comm:message key="manager.teleconsultation" />
</title>
	  ...
	  function myAlert(){
		...
		function cancelFun(){
			jConfirm("<comm:message key='js.confirm_cancel' args='comm.modify'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm) {
					location='<%=request.getContextPath()%>/meeadIndex/icuVisitQuery';
				} else {
					return false;
				}
			});
...
<div id="center">
	<comm:navigator position="manager.teleconsultation.consultationAudit>>application.consultation.modify" />
	<div style="overflow-x:hidden;" id="main" >
<form method="post"   action="<%=request.getContextPath()%>/meeadmdbd/editICUVisitForPass"  onsubmit="return check()">
				...
				<tr>
				    <td colspan="2">
						<table background-color="#ddeefa" width="100%">
							...
							<tr height="3"><td colspan="2">&nbsp;</td></tr>
							<tr>
								<td  align="right" style="vertical-align:middle;">&nbsp;&nbsp;<comm:message key='icu.mobile.devices'/>:<span class="required">*</span></td>		
					            <td  style="vertical-align:middle;">
					    	      <input type="text" id="mobileDevices" name="mobileDevices" style="width:160px;" value="${mmd.devicesNo}"/>
					            </td>
							</tr>
						</table>
				    </td>  
	    ...
	    <div class="subinfor" id="patientinfor2">
			<table width="100%" border="0" cellspacing="2" cellpadding="1" class="meetingApp">
				...
				<tr>
					...
					<td><input type="text" name="expectedtime" id="starttime" value="<fmt:formatDate value="${app.expectedTime}" pattern="yyyy-MM-dd HH:mm" />" onclick="WdatePicker({dateFmt:'yyyy-M-d H:mm',minDate:'%y-%M-%d %H:%m:%s',maxDate:'{%y+1}-%M-%d 21:00:00'})"	style="border: 1px solid #7F9DB9;height:20px;width:175px;background:#fff url(<%=request.getContextPath()%>/resources/My97DatePicker/skin/datePicker.gif) no-repeat right;"	onfocus="this.blur()" size="25" /></td>
				</tr>
				<tr height="3"><td colspan="2">&nbsp;</td></tr>
				<tr>
				    <td colspan="2">
