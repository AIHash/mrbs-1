...
<head>
	...
	<title><comm:message key="manager.icuMonit.Evaluation" /></title>
			...
			jConfirm("<comm:message key='js.confirm_delete' args='manager.icuMonit'/>\n\n<comm:message key='admin.click_ok'/>", "<comm:message key='meeting.m.infotishi'/>",function(resultConfirm){
				if(resultConfirm){
			    	$.post('${pageContext.request.contextPath}/meeadmdbd/delMeeting/' + meetingId, function(text) {
						if(text=='succ'){
							window.location.href = "<%=request.getContextPath()%>/icumonitor/adminIcuMonitorEvaluationQuery";
						} else if(text=='error'){
...
<body onload="heigthReset(64);" onresize="heigthReset(64);" style="padding:0; margin:0;background:url('../resources/images/theme/bg_right-2.gif');">
	<div id="center">
		<comm:navigator position="manager.icuMonit>>manager.icuMonit.Evaluation" />
		<div id="main">
		<form name="MyForm" action="${root}/icumonitor/adminIcuMonitorEvaluationList" method="post" onsubmit="return check()" target="Report">
		...
		<table align="center" width="697">
			...
			<tr>
				<td colspan="3" class="table_middle_bg"><iframe id="dataFrame" align="top" frameborder="0" name="Report" width="700"scrolling="no" src="${root}/icumonitor/adminIcuMonitorEvaluationList"></iframe></td>
			</tr>
		</table>
	</form>
	</div>
