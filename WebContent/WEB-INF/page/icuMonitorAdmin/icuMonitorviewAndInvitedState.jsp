			                  ...
			                  <div style="width:200px;">
							    ...
							    <c:if test="${fn:length(myfilelist.name) > 10}">
								    <div id="myFileListName" style="float:left;width:80px;">
								       <a style="color:blue;background:snow;" 	href="#" onclick="Exp('${myfilelist.path}','${myfilelist.name}');"  title="${myfilelist.name}">${fn:substring(myfilelist.name, 0, 10)}...</a>
								    </div>
							    </c:if>
							    <c:if test="${fn:length(myfilelist.name) <= 10}">
								    <div id="myFileListName" style="float:left;width:50px;">
								       <a style="color:blue;background:snow;" 	href="#" onclick="Exp('${myfilelist.path}','${myfilelist.name}');"  title="${myfilelist.name}">${myfilelist.name}</a>
								    </div>
							    </c:if>
							  </div>	
			            	</li>
		...
		<div class="subinfor" id="patientinfor1">
	        <table class="submit_table" width="80%" id="firstTable" style="table-layout:fixed;positon:fixed;" cellspacing="0" cellpadding="0" border="0">
             ...
             <c:if test="${meetingApplication.state == MEETING_APPLICATION_STATE_PENDING}">
                  <tr height="20"><td align="center" style="font-size:18px;"><comm:message key="meeting.has.been.no.start"/></td></tr>
             </c:if>			       		 
             <c:if test="${meetingApplication.state == MEETING_APPLICATION_STATE_REFUSED}">
                  <tr height="20"><td align="center" style="font-size:18px;"><comm:message key="meeting.has.been.refuse"/></td></tr>
             </c:if>			       		 
	        </table>
		</div>   
	    <div class="subtitle" style="background-color:#dee;">
