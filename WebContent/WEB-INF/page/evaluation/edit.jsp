...
<body leftmargin="0" topmargin="0" onLoad="focus('form','name')" >
	<table border="0" cellspacing="0" cellpadding="0" align="center" class="table_98per">
	    <comm:navigator position="admin.title_system_manage>>admin.evaluation_manage>>admin.evaluation_update" />
		<td colspan="3" height="15" class='queryBackground'>&nbsp;</td>
	</table>

	<form name="form" action="${root}/evaluation/update" method="post" onsubmit="return check()">
	<table border="0" cellspacing="0" cellpadding="0" align="center" class="table_98per">
		<tr>
		...
		<tr>
					<td class="text_left_red2" height="25"  align="right"><comm:message key="admin.user_type"/>:</td>
			<td>&nbsp;<select id="usertype" name ="userType" style='width:150px' >
						<c:forEach items="${USER_TYPE}" var="type" varStatus="var">
							<option value="${type.value}" <c:if test="${evaluation.userType.value == type.value}">selected</c:if>><comm:message key="${type.name}"/></option>
						</c:forEach>      
					</select>
			</td>
			</tr>
